...
User Function MBRFAT11()
	Local cDesc1         := "Este programa tem como objetivo imprimir relatorio "
	Local cDesc2         := "de acordo com os parametros informados pelo usuario."
	Local cDesc3         := "Relatorio de Vendas/Faturamento por Vendedor por Período"
	Local cPict          := ""
	Local titulo         := "Fat x Vend"
	Local nLin           := 80
	Local Cabec1		 := "REF             DESCRICAO                                        QUANT         VALOR    PRC MED     QUANT       VALOR        LL MEDIO      LL TOTAL        MARKETING        DIRETORIA        INVESTIMENTO          ERRO "
	...
	Private limite       := 220
	Private tamanho      := "G"
	Private nomeprog     := "MBRFAT11" // Coloque aqui o nome do programa para impressao no cabecalho
	Private nTipo        := 18
	Private aReturn      := { "Zebrado", 1, "Administracao", 2, 2, 1, "", 1}
	Private nLastKey     := 0
	Private cPerg        := "MBRFAT11  "
	Private cbtxt        := Space(10)
	Private cbcont       := 00
	Private CONTFL       := 01
	Private m_pag        := 01
	Private wnrel        := "MBRFAT11"	// Coloque aqui o nome do arquivo usado para impressao em disco
	Private cString		 := "SD2"
	ValidPerg()
...
//EndIf
	...
	cQuery	+= "		d.C5_TPFRETE,  " +CRLF
	cQuery	+= "		d.C5_FRETEMB, " +CRLF
	...
	cQuery	+= "		(SELECT SUM(g.C6_QTDVEN) " +CRLF
	cQuery	+= "			FROM  " +CRLF
	cQuery	+= "				"+RETSQLNAME("SC6")+" g LEFT JOIN " +CRLF
	cQuery	+= "				"+RETSQLNAME("SC5")+" h ON " +CRLF
	cQuery	+= "					g.C6_NUM = h.C5_NUM " +CRLF
	cQuery	+= "					and h.D_E_L_E_T_ = '' " +CRLF
	cQuery	+= "			WHERE " +CRLF
	cQuery	+= "				g.D_E_L_E_T_ = '' " +CRLF
	cQuery	+= "				and g.C6_NUM = a.D2_PEDIDO " +CRLF
	cQuery	+= "				and g.C6_PRODUTO = a.D2_COD " +CRLF
	cQuery	+= "				and h.C5_EMISSAO BETWEEN '"+dtos(MV_PAR01)+"' and '"+dtos(MV_PAR02)+"' " +CRLF
	cQuery	+= "		) C6_QUANT, " +CRLF

	cQuery	+= "		(SELECT SUM(g.C6_VALOR) " +CRLF
	cQuery	+= "			FROM  " +CRLF
	cQuery	+= "				"+RETSQLNAME("SC6")+" g LEFT JOIN " +CRLF
	cQuery	+= "				"+RETSQLNAME("SC5")+" h ON " +CRLF
	cQuery	+= "					g.C6_NUM = h.C5_NUM " +CRLF
	...
	cQuery	+= "		ISNULL(c.C6_ZZPLLM,0) C6_ZZPLLM, " +CRLF
	...
	cQuery	+= "		f.F4_CODIGO		=	a.D2_TES	and   " +CRLF
	...
	cQuery	+= "		d.C5_NUM		=	a.D2_PEDIDO	and  " +CRLF
...
//cQuery	+= "		c.C6_ZZVLDIR	> 0			and " +CRLF
	cQuery	+= "		c.D_E_L_E_T_	<> '*'  LEFT JOIN  " +CRLF
	...
	cQuery	+= "		d.C5_VEND1, a.D2_COD  " +CRLF

	MemoWrite("\sql\MBRFAT11.SQL",cQuery)
	...
	While !TRA->(EOF())
		...
		If !cVend1 = TRA->C5_VEND1
			...
			@nLin,059 Psay PadR(Transform(nTVQtd	, "@E 9,999,999.99"),15)
			@nLin,093 Psay PadR(Transform(nTVQtdF	, "@E 9,999,999.99"),15)
			@nLin,125 Psay PadR(Transform(Round(nTVLL / nTVVlr * 100,4)	, "@E 999.9999"),15)
			@nLin,151 Psay PadR(Transform(Round(nTVMkt	/ nTVVlr * 100,2), "@E 999.99"),6)
			@nLin,169 Psay PadR(Transform(Round(nTVDir	/ nTVVlr * 100,2), "@E 999.99"),6)
			@nLin,187 Psay PadR(Transform(Round(nTVInv	/ nTVVlr * 100,2), "@E 999.99"),6)
			@nLin,205 Psay PadR(Transform(Round(nTVErr	/ nTVVlr * 100,2), "@E 999.99"),6)
			nLin++
			@nLin,067 Psay PadR(Transform(nTVVlr	, "@E 999,999,999.99"),15)
			@nLin,085 Psay PadR(Transform(Round(nTVVlr/nTVQtd,2) , "@E 9,999.9999"),12)
			@nLin,105 Psay PadR(Transform(nTVVlrF	, "@E 999,999,999.99"),15)
			@nLin,134 Psay PadR(Transform(nTVLL	, "@E 99,999,999.9999"),15)
			@nLin,158 Psay PadR(Transform(nTVMkt, "@E 9,999,999.99"),12)
			@nLin,176 Psay PadR(Transform(nTVDir, "@E 9,999,999.99"),12)
			@nLin,194 Psay PadR(Transform(nTVInv, "@E 9,999,999.99"),12)
			@nLin,207 Psay PadR(Transform(nTVErr, "@E 9,999,999.99"),12)
		...
		While !TRA->(EOF()) .and. cCodigo = TRA->D2_COD .and. cVend1 = TRA->C5_VEND1
			If lAbortPrint
			...
			nQuantF	+=	TRA->C6_QUANT
			nValTotF+=	TRA->C6_VALOR
...
// 	EndIF
	...
	If !cVend1 = TRA->C5_VEND1
		...
		@nLin,001 Psay "TOTAL DO VENDEDOR : "
		@nLin,059 Psay PadR(Transform(nTVQtd	, "@E 9,999,999.99"),15)
		@nLin,093 Psay PadR(Transform(nTVQtd	, "@E 9,999,999.99"),15)
		@nLin,125 Psay PadR(Transform(Round(nTVLL / nTVVlr * 100,4)	, "@E 999.9999"),15)
		@nLin,151 Psay PadR(Transform(Round(nTVMkt	/ nTVVlr * 100,2), "@E 999.99"),6)
		@nLin,169 Psay PadR(Transform(Round(nTVDir	/ nTVVlr * 100,2), "@E 999.99"),6)
		@nLin,187 Psay PadR(Transform(Round(nTVInv	/ nTVVlr * 100,2), "@E 999.99"),6)
		...
		@nLin,067 Psay PadR(Transform(nTVVlr	, "@E 999,999,999.99"),15)
		@nLin,085 Psay PadR(Transform(Round(nTVVlr/nTVQtd,2) , "@E 9,999.9999"),12)
		@nLin,105 Psay PadR(Transform(nTVVlr	, "@E 999,999,999.99"),15)
		@nLin,134 Psay PadR(Transform(nTVLL	, "@E 99,999,999.9999"),15)
		@nLin,158 Psay PadR(Transform(nTVMkt, "@E 9,999,999.99"),12)
		@nLin,176 Psay PadR(Transform(nTVDir, "@E 9,999,999.99"),12)
		@nLin,194 Psay PadR(Transform(nTVInv, "@E 9,999,999.99"),12)
	...
	@nLin,125 Psay PadR(Transform(nTotVll/nTotVlr*100, "@E 9999.99"),7)
	@nLin,151 Psay PadR(Transform(nTotMkt/nTotVlr*100, "@E 9999.99"),7)
	@nLin,169 Psay PadR(Transform(nTotDir/nTotVlr*100, "@E 9999.99"),7)
	@nLin,187 Psay PadR(Transform(nTotInv/nTotVlr*100, "@E 9999.99"),7)
	@nLin,205 Psay PadR(Transform(nTotErr/nTotVlr*100, "@E 9999.99"),7)
	nlin++
	@nLin,067 Psay PadR(Transform(nTotVlr	, "@E 999,999,999.99"),14)
	@nLin,085 Psay PadR(Transform(nTotVlr/nTotQtd	, "@E 9,999.9999"),10)
	@nLin,105 Psay PadR(Transform(nTotVlrF	, "@E 999,999,999.99"),14)
	@nLin,134 Psay PadR(Transform(nTotVll, "@E 999,999,999.99"),14)
	...
	@nLin,207 Psay PadR(Transform(nTotErr, "@E 9,999,999.99"),12)
